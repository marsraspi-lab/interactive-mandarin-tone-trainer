...
function extractPitchCurve(samples, sampleRate, detector = detectPitchAMDF, yinThreshold = 0.25, useBandpass = false) {
  const pitches = [];
  for (let start = 0; start + FRAME_SIZE <= samples.length; start += HOP_SIZE) {
    let frame = samples.slice(start, start + FRAME_SIZE);
    // Apply Butterworth bandpass pre-filter
    if (useBandpass) {
      frame = applyButterworthBandpass(frame, sampleRate);
    }
    // Pass threshold when using YIN (first arg ignored by AMDF)
    const pitch = detector === detectPitchYIN
      ? detector(frame, sampleRate, yinThreshold)
      : detector(frame, sampleRate);
    pitches.push(pitch);
  }
  return pitches;
}

function fullPipeline(samples, sampleRate, detector = detectPitchAMDF) {
  const raw = extractPitchCurve(samples, sampleRate, detector);
...
function fullPipelineRaw(samples, sampleRate, detector = detectPitchAMDF, useBandpass = false, useFBEMA = false) {
  const raw = extractPitchCurve(samples, sampleRate, detector, 0.25, useBandpass);
  const octaveCorrected = applyOctaveCorrection(raw);
  const interpolated = applySplineInterpolation(octaveCorrected);
  const smoothed = applyThreePointSmoothing(interpolated);
  if (useFBEMA) {
    return applyForwardBackwardEMA(smoothed);
  }
  return smoothed;
}
...
function main() {
  ...
  const useYIN = args.includes('--yin');
  const voicedOnly = args.includes('--voiced-only');
  const useBandpass = args.includes('--bandpass');
  const useFBEMA = args.includes('--fb-ema');
  const detector = useYIN ? detectPitchYIN : detectPitchAMDF;
  const detectorName = useYIN ? 'YIN' : 'AMDF';

  console.log('╔══════════════════════════════════════════╗');
  console.log('║   AUTORESEARCH — Training Evaluation    ║');
  console.log('╚══════════════════════════════════════════╝');
  console.log(`   Detector: ${detectorName}`);
  if (useBandpass) console.log('   Pre-filter: Butterworth bandpass 60-400 Hz');
  if (useFBEMA) console.log('   Post-filter: forward-backward EMA (α=0.3)');
  ...
  for (const stem of allStems) {
    ...
    try {
      ...
      jsRaw = fullPipelineRaw(samples, sampleRate, detector, useBandpass, useFBEMA);
    } catch (err) {
